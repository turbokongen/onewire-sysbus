...
class OneWireHub:
    ...
    async def initialize(self, config_entry: ConfigEntry) -> None:
        ...
        if self.type == CONF_TYPE_SYSBUS:
            ...
            _LOGGER.warning(
                "Using the 1-Wire integration via SysBus is deprecated in Home-Assistsant."
                "This HACS component is a relplica of the onewire integration before "
                "it went dead, so that people can still use 1-wire via sysbus, due to "
                "issues with owfs server."
            )
